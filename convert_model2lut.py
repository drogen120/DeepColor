...
# LUT_table = np.zeros((33, 33, 33, 3))

def update_lut(input_images, pre_images, LUT_table):
    imgs_index = input_images / 0.030304
    imgs_index = np.minimum(imgs_index, 32)
    imgs_index = imgs_index.astype(int)
    for k in range(imgs_index.shape[0]):
        i_index = imgs_index[k,:,:,:]
        i_preimg = np.minimum(pre_images[k,:,:,:], 1.0)
        for i in range(imgs_index.shape[1]):
            for j in range(imgs_index.shape[2]):
                index = i_index[i,j,:]
                LUT_table[index[0], index[1], index[2], :] = i_preimg[i,j,:]

    return LUT_table

def write_lutfile(LUT_table):
    with open("./lut_generate.dat", "wb") as lutfile:
        lutwrite = csv.writer(lutfile, delimiter = " ")
        lutwrite.writerow([])
        lutwrite.writerow(["3DLUTSIZE","33"])
        lutwrite.writerow([])
        # lut_reshape = np.reshape(LUT_table, (-1, 3))
        for i in range(LUT_table.shape[0]):
            for j in range(LUT_table.shape[1]):
                for k in range(LUT_table.shape[2]):
                    lutwrite.writerow(LUT_table[i,j,k,:])
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with slim.arg_scope(arg_scope):
            predictions, end_points = color_net.net(input_tensor, is_training=False)
        ...
        with slim.queues.QueueRunners(sess):
            ...
            while (i < dataset.record_numbers):
                ...
                LUT_table = update_lut(input_images, pre_images, LUT_table)
                print i
                if i > 500:
                    break
