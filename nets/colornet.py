...
def color_net(inputs,
    ...
    with tf.variable_scope(scope, 'color_net', [inputs], reuse=reuse):
        net = slim.repeat(inputs, 2, slim.conv2d, 16, [1, 1], scope='conv1')
        end_points['block1'] = net
        # net = slim.max_pool2d(net, [2, 2], scope='pool1')
        # Block 2.
        net = slim.repeat(net, 2, slim.conv2d, 32, [1, 1], scope='conv2')
        end_points['block2'] = net
        # net = slim.max_pool2d(net, [2, 2], scope='pool2')
        # Block 3.
        net = slim.repeat(net, 2, slim.conv2d, 64, [1, 1], scope='conv3')
        end_points['block3'] = net
        # net = slim.max_pool2d(net, [2, 2], scope='pool3')
        colormask = slim.conv2d(net, 12, [1, 1], activation_fn=None, scope='conv4')
...
color_net.default_image_size = 1024
class ColorNet(object):
    default_params = ColorNetParams(img_shape = (1024, 1024))
